...
当前版本：v2026.09.10-beta2 | 最后更新：2026-09-10
...
| 物料编码平台 | ✅ 完成 | 全物料 ERP 8 位料号：ERP 存货分类目录 + 按编码段自动取号发放 + 厂商/单位维护 + 批量导入（Issue #13） |
...
| 日期 | 版本 | 说明 |
|------|------|------|
| 2026-09-10 | v2026.09.10-beta2 | feat(ui): 项目详情技术协议面板支持补充协议—下拉选项改为按已提交文档动态发现，「有补充协议」标记（Issue #8） |
| 2026-09-10 | v2026.09.10-beta1 | fix(api,ui): 物料目录分类树「原制料」显示名改为「原材料」 |
| 2026-09-09 | v2026.09.09-beta7 | docs(ui,api): 物料编码平台文案与文档收尾：全物料口径统一，补 manufacturer 字段、ERP 分类树与审计类目说明（Issue #13） |
| 2026-09-09 | v2026.09.09-beta6 | feat(ui): 物料编码发放页改版（建议码实时预览置顶居中、字段布局优化），查重提示按候选逐行显示编码与名称（Issue #13） |
| 2026-09-09 | v2026.09.09-beta5 | feat(api,ui): 物料目录左侧分类树镜像 ERP 存货分类重排（原制料等 7 大类、空分类灰显），修正空分类点击误显示全库（Issue #13） |
| 2026-09-09 | v2026.09.09-beta4 | feat(api,ui,db): 物料编码平台扩为全物料目录，支持ERP 7大类分类浏览、厂商单位维护与批量导入回填（Issue #13 reopen） |
| 2026-09-08 | v2026.09.08-beta2 | feat(api,ui): 产品文档页支持「导入模板文档」— 手动移除的可选模板文档可勾选一键恢复并重新上传（Issue #17） |
| 2026-09-08 | v2026.09.08-beta1 | feat(api,ui): 产品管理命名规范支持同规格板卡的功能标记与硬件改版（Issue #16）— 新建三级产品可选自动附功能字母(A/B/C)区分同规格不同功能板(如LP2120A-V1)、对既有分支硬件改版版本+1(如LP2120-V2)、改版自动继承上一版产品名并附新版本号；修复命名对话框版本号丢失致同规格撞唯一编号，改由后端按(base,功能标记)事务内重算编号落库 |
| 2026-09-07 | v2026.09.07-beta4 | feat(api,ui): 新增结构件物料编码平台 — 8位料号按编码段自动取号发放、目录化检索替代Excel维护（Issue #13） |
| 2026-09-07 | v2026.09.07-beta3 | fix(api,ui): 板卡时间线支持录入交付时刻并按业务日期归位，交付事件展示交付人/批次数量，归属人随维修Bug责任人流转 |
| 2026-09-07 | v2026.09.07-beta2 | fix(api,ui): 板卡交付时间线按业务日期排序修复，责任人显示修正，交付明细产品编号可点击跳板卡时间线 |
| 2026-09-07 | v2026.09.07-beta1 | fix(api,ui): 任务编辑删除成员后保存仍恢复 — 多选器 tag ×移除不同步 onChange，提交读旧镜像值；任务历史记录不再误记未变更的开始/截止日期（date 与 str 恒不等）|
| 2026-09-04 | v2026.09.04-beta1 | feat(auth): 新增「软件版本维护」细分权限并授予 FPGA 开发角色，可锁定/切换版本来源，普通开发无需完整项目编辑权限 |
| 2026-09-03 | v2026.09.03-beta15 | feat(ui): 新增跨项目「板卡管理」总览页，状态流转对登录用户开放，列表自适应填满视口 |
| 2026-09-03 | v2026.09.03-beta10 | perf(api,ui): 页面打开提速（Issue #12）— ①项目详情打开即渲染基本信息，文档扫描改为打开后后台执行（no_scan+按项目30s节流+待扫队列），切文档tab展示最新（1.9s→27ms）；②项目/产品详情的公告、规格书等外链文档改懒加载占位，不再整页等待；③数据库管理页远端NAS文件列表60s TTL缓存、/backups秒回（冷缓存sync显示待校验，远端后台单次扫描后补全徽标），系统管理页 ~12.7s→秒开；同步写操作改实时拉取远端列表，修复冷缓存 None 崩溃 |
| 2026-09-03 | v2026.09.03-beta9 | fix(ui): 项目文档表新增批量操作 — 序号前加复选框，可选文档可批量软移除、孤儿/手动文档可批量删除（模板文档不可勾选防误删）；任务详情全选修复：空阶段占位行不再携带重复 id:0，DataTable 全选按可勾选行计数，可正常取消全选，批量取消同步清空表格勾选态 |
| 2026-09-03 | v2026.09.03-beta8 | fix(ui): 用户中心任务/项目过滤改按项目编号匹配，修复同名项目选 PE0456 时混入 PE0461 等任务 |
| 2026-09-03 | v2026.09.03-beta7 | feat(api): Bug 责任人变更自动把原责任人加入抄送并同步关注（Issue #10），历史记录的抄送人/负责人显示中文名 |
| 2026-09-03 | v2026.09.03-beta6 | feat(ui): 软件版本页优化（Issue #2）— 版本汇总改标准 DataTable 表格组件，产品基础版本各产品独立卡片明显区分 |
| 2026-09-03 | v2026.09.03-beta2 | refactor(api): 硬件自研单板判定改按产品所属分类 — FPGA发布跟踪不再依赖模板名含原理图/PCB，改为产品树中根分类为自研单板即纳入 |
| 2026-09-03 | v2026.09.03-beta1 | feat(api,ui): 软件版本支持从关联产品选择产品基础版本（Issue #2）— 版本维护/版本汇总双视图，项目发布子项可逐个切换版本来源，硬件产品FPGA文档按产品拆分跟踪各自基础版本，版本汇总展示来源与产品编号 |
| 2026-09-02 | v2026.09.02-beta3 | feat(ui): 工时占比与进度滑块支持滚轮微调、直接输入数值精准设置，输入仅限数字并自动纠偏、回车确认 |
| 2026-09-02 | v2026.09.02-beta2 | fix(api,ui): 支持当天未完整打卡时先记录工时，忘打卡/补卡日按 8h 暂计，企微数据到位自动核正（Issue #9） |
| 2026-09-02 | v2026.09.02-beta1 | fix(api): 项目总览「项目编号」列按数值排序，修复 LSJ 等三位前缀项目排序不生效的问题 |
| 2026-08-31 | v2026.08.31-beta3 | feat(ui): Bug 提交模板改为富文本编辑，创建 Bug 时选模板自动填充富文本内容 |
| 2026-08-31 | v2026.08.31-beta2 | feat(api,ui): 项目新增「老项目跟踪」标记，标记项目不自动按模板建任务，总览可单独筛选（Issue #6） |
| 2026-08-31 | v2026.08.31-beta1 | style(ui): 移除「项目总览」导航栏的待处理告警数字角标 |
...
| 2026-08-27 | v2026.08.27-beta16 | style(ui): 工时日历绿底+绿点改为白色圆环 — 当日历单元格底色为绿色(整日8h~8.5h)且记录工时圆点为绿色(100%记录)时,绿点融入绿底看不清,改为白色圆环(白描边透明中心),红点场景不变 |
| 2026-08-27 | v2026.08.27-beta16 | style(ui): 工时日历绿底+绿点改为白色圆环 — 当日历单元格底色为绿色(整日8h~8.5h)且记录工时圆点为绿色(100%记录)时,绿点融入绿底看不清,改为白色圆环(白描边透明中心),红点场景不变 |
| 2026-08-27 | v2026.08.27-beta15 | fix(ui): 工时日历出差/外出日恢复原点标记 — 日历单元格"记录工时 vs 打卡工时"红/绿原点改为只要有打卡工时(hasCheckin)即显示(外出/出差等免打卡日也记录工时内容,同样显示原点),不再因 isLeave 隐藏 |
| 2026-08-27 | v2026.08.27-beta14 | fix(api,ui): 工时详情出差8h标记+外出按审批时间去午休 — ①工时详情弹窗:出差工作日显示"打卡 8h(出差默认8h)",不再显示免打卡(仅0h的免打卡日才显示免打卡);②外出:后端 get_checkin_calendar 对含有效"外出"审批日 total=审批span-WECOM_LUNCH_HOURS(整日span>=9h扣1.5h午休,与打卡口径一致),打卡时间以审批时间范围为准;③请假仍按实际打卡口径 |
| 2026-08-27 | v2026.08.27-beta13 | fix(api,ui): 企微打卡日历 出差按工作日/周末区分+外出提示修正 — ①出差日:工作日公司默认当天工时固定8h(视为企微打卡),周末默认0h(以实际补卡checkin为准),后端 get_checkin_calendar 按 weekday<5 设 total=8.0/checkin;②日历 hover 提示:免打卡日(出差/外出/请假等)由笼统"请假/免打卡"改为显示实际审批类型(出差/外出/请假 + 免打卡),新增 _leaveTypeTip |
| 2026-08-27 | v2026.08.27-beta13 | fix(api,ui): 企微打卡日历 出差按工作日/周末区分+外出提示修正 — ①出差日:工作日公司默认当天工时固定8h(视为企微打卡),周末默认0h(以实际补卡checkin为准),后端 get_checkin_calendar 按 weekday<5 设 total=8.0/checkin;②日历 hover 提示:免打卡日(出差/外出/请假等)由笼统"请假/免打卡"改为显示实际审批类型(出差/外出/请假 + 免打卡),新增 _leaveTypeTip |
| 2026-08-27 | v2026.08.27-beta12 | fix(api,ui): 企微打卡日历 出差免打卡+外出提示修正(初版,按"出差免打卡0h"实现,后beta13修正为按工作日/周末区分) — ①出差日:出差期间不具备企微打卡条件,无需正常打卡,后端 get_checkin_calendar 对含有效"出差"审批日 total_hours 置0;②日历 hover 提示:免打卡日由笼统"请假/免打卡"改为显示实际审批类型,新增 _leaveTypeTip |
...
| 2026-08-24 | v2026.08.24-beta24 | feat: 请假跨天/撤销/半天正确处理 — ①跨天请假(如7/06-08共3天)按天展开:新增_approval_day_periods按slice_info.day_items逐天生成,每天独立标记免打卡(不再全记到开始日);②撤销(4)/驳回(3)审批periods为空不标记;③半天/部分请假(all_day=false,请假时长<8h):不判整天免打卡,该日仍需上班打卡(工时只算真实打卡);前端_isLeaveDate要求all_day=true才整天免打卡;审批工时按天核算(请假slice.duration去午休)避免跨天57.5h |
| 2026-08-24 | v2026.08.24-beta22 | feat: 企微请假审批识别+免打卡显示+工时纠正 — ①审批识别放宽到请假/外出/外勤/出差/调休/事假/病假(原只识别外出),支持Vacation控件解析请假起止时间;②免打卡日:审批工时不作为"打卡"(total只算真实打卡),日历灰色斜纹显示(新建--hatch变量深浅主题自适应),不显示红边框/圆点;③请假工时按天核算用slice_info.duration(去午休,每天8h),避免date_range跨度跨天异常(如7/06误算57.5h);④免打卡判定status1/2(审批中/通过),撤销4/驳回3不计;前端_isLeaveDate同步status1/2 |
| 2026-08-24 | v2026.08.24-beta15 | feat: 企微打卡同步窗口按触发方式区分 — sync_wecom_data加days参数来区分回看窗口:每日定时同步(full_sync)保持60天,周日定时同步扩到180天(最近6个月,datetime.weekday()==6判断),system-manage/config企微数据源手动同步(sync.py单源)固定180天;解决6/26前打卡不再同步更新问题 |
