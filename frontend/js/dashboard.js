...
function _initDashDt() {
  ...
  _dashDt = new DataTable({
    ...
    columns: [
      ...
      { key: 'end', title: '计划完成', width: '10%', minWidth: 100, headerRender: function() { return '<span style="cursor:pointer" onclick="dashFilter.toggleSortEnd()">计划完成</span> <span id="sort-end-ind" style="color:var(--muted)">⇅</span>'; }, render: function(v) { return '<span style="font-size:12.5px;color:'+(v?'var(--muted)':'var(--warn)')+'">'+(v?formatDate(v):'长期')+'</span>'; } },
      { key: 'status', title: '状态', width: '5%', minWidth: 80, render: function(v) { return renderPill(v); } },
      { key: 'progress', title: '进度', width: '10%', minWidth: 60, render: function(v) { return renderProgressCircle(parseFloat(v)||0, 32, {label:''}); } },
      { key: 'risk', title: '风险', width: '5%', minWidth: 65, render: function(v, row) { var rl=row.risk_level||'normal'; var labels={normal:'正常',low:'较低',medium:'中等',high:'高',overdue:'已超期',incomplete:'资料不全'}; var colors={normal:'var(--success)',low:'var(--muted)',medium:'var(--warn)',high:'var(--danger)',overdue:'var(--danger)',incomplete:'var(--warn)'}; var bgs={normal:'var(--success-lt)',low:'var(--bg)',medium:'var(--warn-lt)',high:'var(--danger-lt)',overdue:'var(--danger-lt)',incomplete:'var(--warn-lt)'}; return '<span class="risk-tag" style="--risk-color:'+(colors[rl]||colors.normal)+';background:'+(bgs[rl]||bgs.normal)+';font-size:11px">'+(labels[rl]||'正常')+'</span>'; } },
      { key: 'linked', title: '关联项目', width: '10%', minWidth: 80, render: function(v, row) { var lp=row.linked_projects; return (lp&&lp.length)?lp.map(function(x){return '<span class="proj-code-btn" style="font-size:10px" onclick="event.stopPropagation();openProject(\''+escHtml(x.code||String(x.id))+'\')" title="'+escHtml(x.name||'')+'">'+escHtml(x.code||x.name)+'</span>';}).join(' '):'<span style="color:var(--muted)">—</span>'; } },
