...
/* 板卡管理操作（board-mgmt.js）共享上下文：本项目交付数据 + 用户/产品表。
   闭包实时读取模块级变量，详情页每次加载/刷新时调用，覆盖总览页注入的 ctx。 */
function _syncBoardMgmtCtx() {
  if (typeof BoardMgr === 'undefined') return;
  BoardMgr.setContext({
    getBoard: function(id) { return _boardById(id); },
    getMeta: function() { return _boardMetaData || { statuses: [], manual_targets: [], schema: {}, repair_statuses: [] }; },
    getUsers: function() { return _userOptions || []; },
    getProducts: function() { return _projectProducts || []; },
    comboCode: function() { return _comboCurCode || ''; },
    displayName: function(u) { return _userDisplayMap[u] || getDisplayName(u); }
  });
}
...
async function loadProjectDetail(code) {
  ...
  try {
    ...
    _syncBoardMgmtCtx();   // 板卡操作数据源指向本项目（覆盖总览页注入的 ctx）
...
// 板卡状态 pill 映射/渲染已提升至 components.js（BOARD_PILL_CLASS/COLORS/boardStatusPill），
// 与「板卡管理」总览页共享单一来源。此处保留别名，兼容 detail.js 既有引用。
var _BOARD_PILL_CLASS = BOARD_PILL_CLASS;
var _BOARD_PILL_COLORS = BOARD_PILL_COLORS;

function _boardPill(status, onClick, title, fx) {
  return boardStatusPill(status, onClick, title, fx);
}
...
function _renderBoardTable(boards) {
  ...
  var cols = [
    ...
    { key: 'actions', title: '操作', width: '120px', minWidth: 120, render: function(v, row) {
      ...
      // 编辑 / 删除需板卡管理权限（admin/board_manage），时间线所有登录用户
      if (_hasBoardPerm()) h += iconEdit('showBoardForm(' + row.id + ')', '编辑');
      if (_hasBoardPerm()) h += iconDelete('deleteBoard(' + row.id + ')', '删除');
      return h;
    }},
  ];
