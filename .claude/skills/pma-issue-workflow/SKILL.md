...
- 告知问题原因
- 告知用户改了什么
- **提供测试链接**：通过 `./server.sh status` 获取当前服务地址和端口，给出可直接点击的验证 URL，使用标准 markdown 链接格式（如 [项目详情](http://192.168.0.100:8000/#/detail/PE0445/docs)），说明需要查看哪个页面/功能来验证修复效果。**禁止对链接使用粗体或其他修饰。**
- 等待用户确认
